    ...
    <view class="container">  
		...
		<view class="user-section">
			...
			<view v-if="!hasLogin">
				...
				<view class="user-info-box"  @click="navToLogin">
				...
				<view class="vip-card-box">
					...
					<view class="b-btn" @click="navToLogin">
						立即开通
					</view>
					<view class="tit">
					...
					<text class="e-b">开通会员享受更多优惠</text>
				</view>
			</view>
			<view v-else>
				...
				<view class="vip-card-box">
					...
					<text class="e-m">会员</text>
					<text class="e-b">您是{{userInfo.level}}会员</text>
				</view>
			</view>
		...
		>
			...
			<view class="history-section icon">
				...
				<list-cell icon="icon-shoucang_xuanzhongzhuangtai" iconColor="#54b4ef" title="我的收藏" border=""></list-cell>
			</view>
		</view>
...
<script>  
	import listCell from '@/components/yt-list-cell';
    import {  
        mapState,  
        mapMutations  
    } from 'vuex';  
    ...
    export default {
        ...
        methods: {  
			...
			share(){
				uni.navigateTo({  
				    	url:this.getUrlType('PAGES_SHARE')
				})  
			},
			/**
			 * 用户地址
			...
			address(){
				uni.navigateTo({  
				    	url:this.getUrlType('PAGES_ADDRESS_LIST')
				}) 
			},
			navToLogin(){
				uni.navigateTo({
			...
			sendRequest: function() {
				this.loading = true
				this.$api.post_request({noncestr: Date.now(),a:'ba',ab:'ab ',aab:' aab'},"ajax/echo/text?name=uni-app").then((res)=>{
					this.loading = false;
			...
			async sendRequest1() {
				this.loading = true
				let res = await this.$api.post_request({noncestr: Date.now()},"ajax/echo/text?name=uni-app");
				this.loading = false;
				this.res = '请求结果 : ' + JSON.stringify(res);
			}
